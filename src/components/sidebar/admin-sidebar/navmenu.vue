<template>
  <div id="sidebar-menu " class="sidebar-content-sub">
    <ul
    ...
    >
      ...
      >
        <a
          ...
          v-if="
            menuItem.type == 'sub' &&
            (menuItem?.userRole == true || extralogedInUserRole?.Admin == true)
          "
        ...
        >
          <li v-for="(childrenItem, index) in menuItem.children" :key="index">
            <router-link
              ...
              v-if="
                childrenItem?.userRole == true ||
                extralogedInUserRole?.Admin == true
              "
...
export default {
  ...
  data() {
    return {
      ...
      text: "",
      active: false,
      extralogedInUserRole: JSON.parse(getUserRole()),
    };
  },
  computed: {
    ...mapState({
