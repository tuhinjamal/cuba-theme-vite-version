  ...
  <b-form class="needs-validation" novalidate="">
    <b-row class="alignment">
      <b-col cols="12" class="bg-light mt-3">
        <h3 class="mt-3 text-black">নতুন জোন সংযোজন</h3>
      </b-col>
    </b-row>
    ...
    <b-row class="mx-2 p-1 rounded border bg-white alignment">
      ...
      <b-col cols="12" md="6" class="mt-3">
        <b-form-group class="text-black" label="জোনের  নাম ">
          <b-form-input
            :disabled="mode == 'view'"
            v-model="form.name"
            placeholder="জোনের  পুরো নাম  লিখুন"
            required
          ></b-form-input>
        </b-form-group>
  ...
  methods: {
    createSection() {
      if (this.form.name == null || this.form.name == "") {
        if (this.form.name == null || this.form.name == "") {
          this.nameValdation = "জোনের  নাম লিখুন ";
        } else {
          this.nameValdation = "";
        }
      } else {
        ...
        if (this.form.name !== null || this.form.name == "") {
          this.$store
            .dispatch("zone/createZone", this.form)
            .then((res) => {
              if (res.status === 200) {
    ...
    updateSection() {
      if (this.form.name == null || this.form.name == "") {
        ...
        if (this.form.name !== null || this.form.name == "") {
          this.$store
            .dispatch("zone/updateZone", this.form)
            .then((res) => {
              if (res.status === 200) {
                this.$router.push(`/zones`);
              }
